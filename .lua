-- Это код для Library в стиле Nil Hub (темно-красный дизайн с акцентами, плавность, UICorner 0.5, секции без фона, toggles с серым фоном при включении, sliders с черной линией прогресса, темно-красным фоном, кружочком смесью 50% черного и 50% темно-красного, TextBox для ввода с точностью чисел без лишних нулей, keybinds исправленные: клик на keybind начинает биндинг без зажима, текст 'Press a key...', нажатие биндит, повторный клик сбрасывает на None, tabs слева без анимации открытия, до 5 табов, ScrollingFrame появляется автоматически если контент больше размера таба (CanvasSize динамически рассчитывается по добавленным элементам, листать можно только до конца контента без пустоты), toggles по умолчанию выключены, кнопка закрытия 'X' в GUI, toggle-кнопка открытия/закрытия GUI отдельно (квадрат с UICorner 0.5, изначально 'OFF', клик открывает GUI и становится 'ON', клик закрывает и 'OFF')).
-- Используй loadstring этого кода в своем скрипте. Затем создай Library, добавь табы (до 5), добавь элементы в табы (секции, toggles, sliders, buttons, dropdowns). Функции элементов (callbacks) ты сам допишешь (например, print или свои хаки). ScrollingFrame управляется автоматически: если элементов мало (например, 1 toggle + 3 кнопки), ScrollingFrame не нужен (CanvasSize = 0, ScrollBarThickness = 0), если больше - появляется и листать можно только до конца (без пустоты).
...
function Library:CreateWindow(title)
    ...
    -- Toggle-кнопка открытия/закрытия GUI (отдельный квадрат, UICorner 0.5, изначально 'OFF')
    local ToggleButton = Instance.new("TextButton")
    ToggleButton.Name = "ToggleButton"
    ToggleButton.Size = UDim2.new(0, 50, 0, 50)
    ToggleButton.Position = UDim2.new(0, 10, 0.5, -25)
    ToggleButton.BackgroundColor3 = Color3.fromRGB(30, 0, 0)  -- Темно-красный фон
    ...
    ToggleButtonUICorner.CornerRadius = UDim.new(0, 8)  -- UICorner 0.5 (но радиус 8 для квадрата)
    ToggleButtonUICorner.Parent = ToggleButton

    -- Главный Frame (темно-красный фон, draggable)
    ...
    -- Контейнеры для табов (ScrollingFrame для каждого)
    local TabFrames = {}
    local TabButtons = {}
    ...
    function Library:AddTab(name)
        ...
        TabFrame.ScrollBarThickness = 0  -- Изначально без скролла
        TabFrame.CanvasSize = UDim2.new(0, 0, 0, 0)  -- Динамически
        ...
        function tab:AddToggle(title, callback)
        ...
        function tab:AddDropdown(title, options, default, callback)
            ...
            yPos = yPos + 60 + #options * 25
            return DropdownFrame
        end

        -- Обновление CanvasSize и ScrollBarThickness после добавления элементов
        ...
        -- Хук для обновления после добавления
        ...
        tab.AddButton = function(self, ...)
            oldAddButton(self, ...)
            updateCanvas()
        end
        local oldAddDropdown = tab.AddDropdown
        tab.AddDropdown = function(self, ...)
            oldAddDropdown(self, ...)
            updateCanvas()
        end
